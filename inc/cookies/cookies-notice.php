            ...
            <div class="cookie__wrapper">
                ...
                <form id="cookie-form" class="cookie__container" method="POST" action="<?php echo admin_url( 'admin-post.php' ); ?>">
                    ...
                    <div class="cookie__buttons">
                        ...
                        <button type="submit" name="accept" class="button cookie__button cookie__button--accept js-cookie-accept"><?php if ( $cookie_accept_label ) { echo $cookie_accept_label; } else { _e( 'Accept', 'kolksluis_solar' ); } ?></button>
                        
                        <?php if ( $cookie_refuse_active ) { ?>
                            <button type="submit" name="refuse" class="button cookie__button cookie__button--refuse js-cookie-refuse"><?php if ( $cookie_refuse_label ) { echo $cookie_refuse_label; } else { _e( 'Refuse', 'kolksluis_solar' ); } ?></button>
                        <?php } ?>

                        <?php if ( $cookie_read_more_active ) { ?>
                            <a href="<?php if ( $cookie_read_more_page ) { the_permalink( $cookie_read_more_page ); } else { echo '#'; } ?>" class="button cookie__button cookie__button--read-more js-cookie-read-more" target="_self"><?php if ( $cookie_read_more_label ) { echo $cookie_read_more_label; } else { _e( 'Cookie policy', 'kolksluis_solar' ); } ?></a>
                        <?php } ?>
        ...
        <?php if ( $cookie_revoke_active ) { ?>
            ...
            <div id="cookie-revoke">
                <form id="cookie-revoke-form" method="POST" action="<?php echo admin_url( 'admin-post.php' ); ?>">
                    ...
                    <input type="hidden" name="_wp_referrer" value="<?php echo home_url( $wp->request ); ?>">
                    <button type="submit" name="revoke" class="button cookie__button cokie__button--revoke js-cookie-revoke"><?php if ( $cookie_revoke_label ) { echo $cookie_revoke_label; } else { _e( 'Revoke cookie', 'kolksluis_solar' ); } ?></button>
                </form>
            </div>
        <?php } ?>
